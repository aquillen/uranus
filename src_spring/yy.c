...
// return index of closest particle to position x,y,z
...
int *marksurface(struct reb_simulation* const r, int N_bennu, double mind){
   ...
   for(int j=N_bennu;j<r->N; j++){
      ...
      int j0 = nearest_to_shape(r,0, N_bennu, x,y, z);
...
// there is a global array of surfacepoint structures called sfp[]
int surface_shape(struct reb_simulation* const r, int i_area, double minr) //ZYH surface points identification

{
    int N_bennu = r->N;
    int i,j,i_long,i_lati,idf;
    int id_spf[i_area][i_area];
    double max_dis[i_area][i_area],rp[3],distrp;
    double angle[2];
    ...
    for (i = 0;i <i_area;i++)
    {
        ...
        {
            max_dis[i][j]=0.0;
            id_spf[i][j]=N_bennu+1;  // start by making this something unreal
        }
    }
    ...
    for (i = 0;i <N_bennu;i++)
    {
        rp[0] = r->particles[i].x;
        rp[1] = r->particles[i].y;
        rp[2] = r->particles[i].z;
        distrp = sqrt(rp[0]*rp[0]+rp[1]*rp[1]+rp[2]*rp[2]);
        potoang(rp,angle); // angle contains both lat and long
        
        //printf("rp: %e %e %e %f %f \n", rp[0],rp[1],rp[2],angle[0],angle[1]);
        
        double fi = i_area;
        
        i_long = floor(angle[0]/(4.0*acos(0.0)/fi));  // acos(0) = M_PI
        i_lati = floor(angle[1]/(2.0*acos(0.0)/fi));
        
        // find points that are largest radius at every lat and long
        if (distrp > max_dis[i_long][i_lati] && distrp > 0.9*minr)
        {
            max_dis[i_long][i_lati] = distrp;
            id_spf[i_long][i_lati] = i;
        }
    ...
    for (i = 0;i <i_area;i++)
    {
        for (j =0;j <i_area;j++)
        {
            ...
            //printf("%d \n",idf);
            if (max_dis[i][j]>0.0 && id_spf[i][j]<=N_bennu) // yes use this point
            {
                sfp[idf].idf = id_spf[i][j];
                sfp[idf].ilong = i;
                sfp[idf].ilati = j;
                sfp[idf].dist = max_dis[i][j];
                idf=idf+1;
            }
    ...
    printf("Total number on surface: %d\n",idf);
    
    FILE *fsp;
    
    fsp = fopen("particle_surface.txt","w");
    ...
    fprintf(fsp,"%lf %lf %lf %d \n",r->particles[i].x,r->particles[i].y,r->particles[i].z,flag);
...
// input v[3] = (x,y,z)  output a[2] = (lat, long)
void potoang(double v[3], double a[2])  // ZYH vector to longitude and latitude
{
    double s1,s2,c1;
    // z = v[2]
    s2 = v[2]/sqrt(v[0]*v[0]+v[1]*v[1]+v[2]*v[2]); // z/r  range [-1,1]
    a[1] = asin(s2);  // [-pi/2 to pi/2]
    double w = sqrt(v[0]*v[0]+v[1]*v[1]);   // w = sqrt(x*x + y*y) 
    c1 = v[0]/w;   // x /w
    s1 = v[1]/w;   // y /w
    a[0] = atan2(s1,c1);  // range [-pi/pi]
    if(a[0] <0.0) a[0] += 2.0*M_PI;
    a[1] += M_PI/2;  // returns [0,pi] note offset!!!!
}

// increase display radius of all surface particles ZYH
// this uses the global array of surfacepoint structures
void surfaceparticle_display(struct reb_simulation* const r, int i_surface)
{
    int ipa = 0;
    int ips = 0;
    for (ipa=0;ipa<r->N;ipa++)
    {
        for (ips=0;ips<i_surface;ips++)
        {
            if (ipa == sfp[ips].idf) r->particles[ipa].r = 0.04; 
        }
    }
}
